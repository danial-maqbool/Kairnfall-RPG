...
## Ground texture repair (2026-09-09)

Grass, moss, marsh, dirt, sand, ash and snow now use low-contrast clustered
surface tones instead of the four-pixel Bayer pattern. All four primary
vegetation variants share matching boundary samples. Grass blades stay clear
of those boundaries. Ordered dither remains available for cloth and other
materials; `forge/brush.py` is unchanged.

`python atelier/build.py --only terrain` and `--manifest` no longer need a game
catalogue. Other catalogue-driven groups still require their real input data.
This permits a terrain-only repair without generating or modifying game content.

Review the mixed-variant and single-variant fields in
`../foundry/preview/terrain.png`. A single repeated 32px image will still repeat
its larger motifs; this repair removes the intrusive four-pixel checker, not
all finite-tileset repetition. See `../foundry/ART_REPAIR_2026-09-09.md`.
