"""Load authored 2D humanoid base sheets into the shared Atelier API.

The authored sheets are the source of truth for the twelve base bodies. If they
are absent in a source-only checkout, the pure 2D illustrated renderer remains
...
from . import illustrated_body, rig
...
def body_frame(build, skin, state, frame, direction):
    ...
    if sheet is None:
        return illustrated_body.body_frame(build_index, skin_index, state, frame, direction)
