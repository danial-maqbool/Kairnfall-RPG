...
internal static class PresentationChecks
{
    public static async Task Run(Node host, GameRoot game, Action<bool, string> check)
    {
        ...
        try
        {
            track.Invoke(world, new object[] { id, position, new Point(1, 0), 100.0, 0.0, false, false });
            foreach (int state in new[] { 2, 3, 4 })
            {
            ...
            track.Invoke(world, new object[] { id, position, new Point(1, 0), 0.0, 0.1, false, false });
            world._Process(.7);
