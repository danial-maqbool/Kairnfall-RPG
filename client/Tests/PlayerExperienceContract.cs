...
public partial class PlayerExperienceContract : Node
{
    ...
    private void Require([DoesNotReturnIf(false)] bool value, string message)
    {
        ...
        checks++; GD.Print("PASS EXPERIENCE: " + message);
    }
    ...
    public override async void _Ready()
    {
        ...
        try
        {
            ...
            Mouse(slot, MouseButton.Left, true, true); Mouse(slot, MouseButton.Left, false);
            Require(activated == 1 && selected == 1, "Native double-click invokes the equipment action once");
            Mouse(slot, MouseButton.Right, true); Mouse(slot, MouseButton.Right, false);
            ...
            var itemControls = game.FindChildren("*", "Control", true, false).OfType<EquipmentItemSlot>().ToArray();
