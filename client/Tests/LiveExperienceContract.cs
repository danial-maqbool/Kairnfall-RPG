...
public partial class LiveExperienceContract : Node
{
    ...
    private EquipmentMenu? ActiveMenu()
        => game.FindChildren("ItemContextMenu", "Control", true, false).OfType<EquipmentMenu>().SingleOrDefault(x => !x.IsQueuedForDeletion());
    private async Task ContextEquipment(string itemId, bool equip)
    {
        await Click(ItemControl(itemId), MouseButton.Right);
        await Until(() => ActiveMenu() is not null, "Native right-click did not open item actions.");
        var menu = ActiveMenu()!;
        var button = menu.GetActionButton(0);
        Require(button.Text == (equip ? "Equip" : "Unequip"), "Context action describes the current authoritative equipment state");
        long sequence = Self.LastAction;
        await Click(button);
        await Until(() => Self.Equipment.GetValueOrDefault("weapon") == (equip ? itemId : null), "Native context action did not update authoritative equipment.");
        await Until(() => !GodotObject.IsInstanceValid(menu), "The context menu retained its native node after selection.");
        await Delay(.2);
        Require(Self.LastAction == sequence + 1, "One native context click produces exactly one authoritative action");
        Require(ActiveMenu() is null, "The completed context menu leaves no active overlay");
    }
    private async Task Capture(string filename)
    {
        await Frame();
        await ToSignal(RenderingServer.Singleton, RenderingServer.SignalName.FramePostDraw);
        Call("SaveScreenshot", filename);
    }
    ...
    public override async void _Ready()
    {
        try
        {
            ...
            await Delay(.3); await Click(ItemControl(weapon)); await Click(ItemControl(weapon), doubleClick: true);
            ...
            Require(true, "A native first click followed by double-click supports equip and unequip");
            await Delay(.3);
            for (int n = 0; n < 5; n++)
            {
                await ContextEquipment(weapon, false);
                await ContextEquipment(weapon, true);
            }
            for (int n = 0; n < 5; n++)
            {
                sequence = Self.LastAction;
                await Click(ItemControl(weapon), MouseButton.Right);
                await Until(() => ActiveMenu() is not null, "Context menu did not open for its close test.");
                var dismissed = ActiveMenu()!;
                await Tap(Key.Escape);
                await Until(() => !GodotObject.IsInstanceValid(dismissed), "Escape did not free the item context menu.");
                Require(Self.LastAction == sequence && Self.Equipment.GetValueOrDefault("weapon") == weapon,
                    "Dismissing item actions does not send an equipment command");
                Require(Field<string>(game, "currentPage") == "Inventory", "Escape dismisses the context menu without closing inventory");
            }
            await ContextEquipment(weapon, false);
            ...
            await Delay(.3); await Capture("07-equipment.png");
            ...
            await Capture("04-player-combat.png");
            ...
            if (pile is not null)
            {
                ...
                if (expectedItems > 0)
                {
                    ...
                    await Capture("10-loot-popup.png");
                }
            }
