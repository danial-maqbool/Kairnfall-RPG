...
public partial class GameRoot
{
    ...
    private EquipmentMenu? itemMenu;
    ...
    {
        if (gameWindow is null || Snapshot is not { } snapshot) return;
        if (itemMenu is not null && GodotObject.IsInstanceValid(itemMenu)) itemMenu.Close();
        selectedItem = item.Id; selectedBag = bag; lastPageStamp = "";
        var definition = Data.Item(item.Template);
        string owner = snapshot.Self.Id;
        var ownerWindow = gameWindow;
        bool equippedWhenOpened = Items.Equipped(snapshot.Self, item.Id);
        var menu = new EquipmentMenu
        {
            Name = "ItemContextMenu", Title = definition.Name,
            Subtitle = item.Rarity + " · " + Ui.Words(definition.Type),
            Icon = Assets.Icon(item.Template), Accent = Ui.RarityColor(item.Rarity),
            ContextValid = () => !closing && Snapshot?.Self.Id == owner
                && GodotObject.IsInstanceValid(ownerWindow) && !ownerWindow.IsQueuedForDeletion()
                && gameWindow == ownerWindow
        };
        if (definition.Slot != "" && bag == "inventory") menu.AddItem(equippedWhenOpened ? "Unequip" : "Equip", 0);
        menu.AddItem("Inspect item", 1);
        if (bag == "bank") menu.AddItem("Withdraw", 3);
        ...
        {
            if (Snapshot is not { } current || current.Self.Id != owner || closing) return;
            var owned = (bag == "bank" ? current.Self.Bank : current.Self.Inventory).FirstOrDefault(x => x.Id == item.Id);
            if (owned is null) { Notify("This item is no longer in the selected bag.", true); return; }
            if (choice == 0)
            {
                if (Items.Equipped(current.Self, owned.Id) != equippedWhenOpened)
                {
                    Notify("Equipment changed. Open item actions again.", true); return;
                }
                ToggleEquipment(owned.Id);
            }
            else if (choice == 1) { selectedItem = owned.Id; selectedBag = bag; lastPageStamp = ""; }
            else if (choice == 2) Send("consume", item: owned.Id);
            else if (choice == 3)
            {
                if (NearRole("banker")) Send("withdraw", item: owned.Id);
                else Notify("Move closer to a banker to withdraw this item.", true);
            }
            else if (choice == 4) Send("read", item: owned.Id);
        };
        itemMenu = menu;
