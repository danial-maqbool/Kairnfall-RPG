...
public partial class GameRoot
{
    public override void _UnhandledInput(InputEvent @event)
    {
        if (@event is InputEventKey { Pressed: true, Echo: false } key)
        {
            if (awaitingBinding != "")
            {
                ...
                if (key.PhysicalKeycode != Key.Escape)
                {
                    ...
                    {
                        Notify("That key already controls " + Ui.Words(conflict) + ". Choose another key.", true);
        ...
        {
            var creature = Snapshot!.Creatures.FirstOrDefault(x => x.Id == hit.Id);
            if (creature is { Health: > 0, Owner: "" })
            {
                autoAttack = true;
                TryBasicAttack();
            }
