...
public partial class ClientAudio : Node
{
    ...
    public override void _Ready()
    {
        ...
        ambience = new AudioStreamPlayer();
        AddChild(music); AddChild(ambience);
        music.Finished += RestartMusic;
        ambience.Finished += RestartAmbience;
        for (int i = 0; i < 8; i++)
        {
    ...
    {
        if (!releasing && IsInsideTree() && GodotObject.IsInstanceValid(music) && music.Stream is not null) music.Play();
    }
    private void RestartAmbience()
    {
        if (!releasing && IsInsideTree() && GodotObject.IsInstanceValid(ambience) && ambience.Stream is not null) ambience.Play();
    }
    public override void _ExitTree()
    {
        // Stop playback before native players and their stream references are released.
        // Do not dispose shared ResourceLoader assets while another node can own them.
        releasing = true;
        if (GodotObject.IsInstanceValid(music))
        {
            music.Finished -= RestartMusic;
            music.Stop(); music.Stream = null;
        }
        if (GodotObject.IsInstanceValid(ambience))
        {
            ambience.Finished -= RestartAmbience;
            ambience.Stop(); ambience.Stream = null;
        }
    ...
    {
        musicVolume = Math.Clamp(melody, 0, 1);
        effectsVolume = Math.Clamp(effects, 0, 1);
        ApplyVolume();
    }
    ...
    private AudioStream? Load(string key)
    {
        ...
        {
            missing.Add(key); GD.PushWarning("Missing audio: " + path); return null;
        }
    ...
    {
        if (region == key || music is null || releasing) return;
    ...
    public void PlayEffect(string action)
    {
        ...
        string key = action switch
        {
            ...
            "loot" or "chest" or "buy" or "sell" => "coins",
            "craft" or "build" => "hammer",
            "equip" or "unequip" or "socket" or "unsocket" => "equip",
            "consume" or "rest" => "drink", _ => "ui"
        };
