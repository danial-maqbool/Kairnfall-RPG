...
public sealed class RealmHost(Catalog catalog,RealmStore store,AccountStore accounts,IHostApplicationLifetime lifetime,ILogger<RealmHost> logger) : BackgroundService
{
    ...
    public async Task<T> WriteAsync<T>(Func<RealmEngine,T> write,CancellationToken cancel)
    {
        ...
        {
            RequireReady();
            var result=write(Engine);
            await PersistAsync(cancel);
            return result;
        }
        catch(RuleException) { throw; }
        catch(Exception e) { FailClosed(e); throw new RuleException("The server could not confirm the operation. Reconnect before retrying."); }
        finally { gate.Release(); }
    }
    ...
    public async Task HandleCommandAsync(Peer peer,GameCommand command,CancellationToken cancel)
    {
        ...
        catch(RuleException e) { peer.Enqueue(new(){Kind="error",Error=e.Message}); }
        catch(Exception e) { FailClosed(e); }
        finally { gate.Release(); }
    }
    private void SendSnapshot(Peer peer)
    {
        var player=Engine.Player(peer.CharacterId);
        var invitations=Engine.State.Parties.Values.Where(x=>x.Invites.Contains(player.Id)).Select(x=>new GroupInvitation{Id=x.Id,Name=x.Name,Leader=x.Leader,Guild=false})
            .Concat(Engine.State.Guilds.Values.Where(x=>x.Invites.Contains(player.Id)).Select(x=>new GroupInvitation{Id=x.Id,Name=x.Name,Leader=x.Leader,Guild=true})).ToList();
        peer.Enqueue(new(){Kind="snapshot",Snapshot=Engine.Snapshot(peer.CharacterId),Loot=Engine.VisibleLoot(peer.CharacterId),Invitations=invitations});
    }
    ...
    public async Task DetachAsync(Peer peer,CancellationToken cancel)
    {
        ...
        catch(Exception e) when(e is not OperationCanceledException) { FailClosed(e); }
        finally { gate.Release(); }
    }
        ...
        try
        {
            while(await timer.WaitForNextTickAsync(stoppingToken))
            {
                ...
                try
                {
                    ...
                    FlushChat();
                    if(ticks%2==0) foreach(var peer in peers.Values) SendSnapshot(peer);
                }
                finally { gate.Release(); }
                double elapsed=watch.Elapsed.TotalMilliseconds;
                lock(tickDurations)
                {
                    tickDurations.Enqueue(elapsed); if(tickDurations.Count>1200) tickDurations.Dequeue();
                    if(elapsed>50) overruns++;
                }
                if(DateTimeOffset.UtcNow>=nextSessionCheck)
                {
                    nextSessionCheck=DateTimeOffset.UtcNow.AddSeconds(30);
                    foreach(var peer in peers.Values)
                        if(!await accounts.IsSessionValidAsync(peer.Session,stoppingToken)) peer.Abort();
                }
            }
        }
    ...
    public object Diagnostics()
    {
        lock(tickDurations)
        {
            ...
            return new{ready,protocol=Wire.Version,online=peers.Count,ticks,overruns,commits,
                ...
                tickP95Ms=values.Length==0?0:Math.Round(values[Math.Min(values.Length-1,(int)(values.Length*0.95))],3)};
        }
    }
