...
Saved raw skill XP stays intact. `PracticeOnlyXp` records the part of future skill gains excluded from overall advancement. Missing legacy fields default to zero. Old levels remain unchanged. Fractional practice and credit persist with the character. All 60 skills at their cap retain the existing terminal overall-level milestone. The prior overall XP-to-level mapping runs on credited training rather than destructively recalculating past progress.
