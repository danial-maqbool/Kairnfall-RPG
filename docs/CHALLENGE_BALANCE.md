...
Direct hits, delayed hits, companion hits, kills and defensive training share the authoritative combat helper. Healing uses the recipient's latest real hostile contact within 15 seconds; other support uses the caster's contact within 10 seconds. Contact records the actual enemy level, not the advertised region level. Later weak contacts replace older strong ones. Expired, foreign-region, dead-recipient and missing legacy contexts grant no support XP, but valid healing still restores health. The serializable context expires against authoritative realm time and clears on death. Contact and reward changes participate in transaction rollback and replay handling. This prevents a low-level fight in a high-level map from bypassing the challenge penalty. Meditation remains noncombat training. The displayed target estimate applies to the selected encounter; support estimates also depend on the recipient's recorded contact.
